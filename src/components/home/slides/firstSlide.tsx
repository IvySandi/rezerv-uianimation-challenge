...
const hiddenUnderMainIconId = "chillin-8";
const foregroundIconIds = new Set([
  "chillin-0",
  "chillin-1",
  "chillin-5",
  "chillin-6",
...
export function FirstSlide({
  ...
  const renderIcon = (icon: (typeof iconAnimation)[number]) => (
    ...
    >
      {icon.id === focusedTransitionIconId ? (
        null
      ) : (
        <Image
          src={icon.src}
  ...
  return (
    <>
      ...
      >
        {backgroundIcons.map(renderIcon)}
      </motion.div>
      <motion.div
        ...
        transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
      >
        {foregroundIcons.map(renderIcon)}
      </motion.div>
    </>
  );
}
